...
\textbf{Weight Update Rule} -- after every wrong prediction, the weights are changed in the direction that would have made the prediction correct:
...
\textbf{Task 1 -- Dataset Exploration:} The dataset was loaded using \texttt{fetch\_ucirepo(id=267)}, and the features and target were combined into a single dataframe. Its shape, missing values, and descriptive statistics were examined before further processing.

\textbf{Task 2 -- Exploratory Data Analysis:} Features histograms, a correlation heatmap, a pairwise scatter plot matrix, and boxplots were generated to understand the shape, spread, and relationships of the four features, along with a class-distribution plot to check class balance.
...
\textbf{Task 6 -- Model Evaluation:} The trained model was evaluated on the test set using accuracy, precision, recall, F1-score, and a confusion matrix.
...
The repository contains the full notebook deep learning lab 1.ipynb,the python file, the \texttt{requirements.txt} needed to run it, and this report's LaTeX source.
...
Out of 275 test samples, 149 authentic notes and 122 forged notes were correctly classified. Only 4 authentic notes were wrongly flagged as forged, and \textbf{not a single forged note was missed} (0 false negatives). Failing to flag a forged
note is a far costlier error than being being more cautious about a genuine note.
...
\end{figure}
\textbf{Observation:} Variance and Entropy look fairly spread out and close to bell-shaped, while Skewness and Curtosis have a longer tail on the right with a few extreme values -- meaning the raw features aren't on a comparable scale, which is exactly why standardizing them later matters.
...
\end{figure}
\textbf{Observation:} Across every pair of features, the two classes form mostly separate clusters, especially along Variance and Skewness, with only a bit of overlap -- meaning that a linear model like the perceptron will work well here.
...
\end{figure}
\textbf{Observation:} Skewness and Curtosis have a fair number of outlier points beyond the whiskers, while Variance and Entropy are more tightly contained -- another point explaining the importance of standardizing before training.
...
\end{figure}
\textbf{Observation:} All three learning rates reach the same final accuracy, but the internal error trajectory is visible more at the highest rate (0.1) and much smoother at the lowest (0.001).
...
\end{figure}
\textbf{Observation:} The Step function jumps sharply between 0 and 1 with no usable slope anywhere, while the Sigmoid curve is smooth and differentiable across its entire range, which makes the  gradient-based training possible.
...
\end{figure}
\textbf{Observation:} The error never drops to zero and stays flat near 3--4 misclassifications out of 4 points for the entire run. This shows direct experimental proof that a single straight-line boundary cannot solve XOR.
...
\section*{10. Extension: Perceptron Learning for Logic Gates}

As a follow-up to the main experiment, the same perceptron learning rule was applied to three classic logic gates -- AND, OR, and NOT -- to observe the weight updates directly on problems small enough to track by hand. Unlike the epoch-level logging used for the main banknote model, this extension records the weights and bias after \textit{every single update}, so the decision boundary can be visualized at each step of learning rather than only at the end of each epoch.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{and_decision_boundary.pdf}
\caption{AND Gate -- Decision Boundary After Each Update}
\end{figure}
\textbf{Observation:} The boundary starts undefined (zero weights cannot separate anything) and only becomes visible from Update 2 onward. Over 8 updates across 3 epochs, the line rotates and shifts until it isolates the single point $(1,1)$ from the other three inputs, matching the AND truth table exactly.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{or_decision_boundary.pdf}
\caption{OR Gate -- Decision Boundary After Each Update}
\end{figure}
\textbf{Observation:} OR converges faster than AND -- only 5 updates across 3 epochs -- since separating a single point $(0,0)$ from the other three is a slightly easier boundary to find than isolating $(1,1)$. The final boundary keeps everything except the origin on the positive side.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{not_decision_boundary.pdf}
\caption{NOT Gate -- Decision Boundary After Each Update}
\end{figure}
\textbf{Observation:} With only a single input feature and two possible values, NOT converges in just 2 updates -- the fastest of the three gates. The final boundary sits at $x_1 = 0$, correctly flipping 0 to 1 and 1 to 0.

\begin{table}[H]
\centering
\caption{Final Weights -- Logic Gate Perceptrons}
\begin{tabular}{lccc}
\toprule
Gate & Final Weights & Final Bias & Updates to Converge \\
\midrule
AND & $[0.2,\ 0.1]$ & $-0.2$ & 8 \\
OR  & $[0.1,\ 0.1]$ & $-0.1$ & 5 \\
NOT & $[-0.1]$ & $0.0$ & 2 \\
...
All three gates are linearly separable, so the single layer perceptron converges cleanly on each one -- a direct contrast to the XOR case discussed in Section 7, where no such convergence is possible.
...
Several noticeale observations were :
...
\item Cross-checking against Scikit-learn's own Perceptron served as a useful validation step, confirming that the from-scratch implementation was identical to the other perceptron.
...
\item The XOR experiment made the linear-separability limitation directly observable rather than purely theoretical -- the error remained permanently stuck. It showed that perceptron can only show linear decision boundaries.
